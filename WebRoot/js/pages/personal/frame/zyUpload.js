	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
				...
				url              : "CropAction.jhtml?pictureType=commentPic",  	// 上传文件的路径
			...
			this.createHtml = function(){
				...
				html += '			<div  class="upload_preview preview">';
				html += '				<div class="add_upload">';
				html += '					<a style="height:60px; width:90px;" title="点击添加文件" class="rapidAddImg" class="add_imgBox" href="javascript:void(0)">';
				html += '						<div class="uploadImg" style="width:90px; height:60px;">';
				html += '							<img class="upload_image" src="img/add_img.png" style="width:90px; height:60px;"/>';
				html += '						</div>';
				html += '					</a>';
				html += '				</div>';
				html += '			</div>';
				...
				html += '			<button type="button" class="fileSubmit" class="upload_submit_btn">确认上传文件</button>';
				html += '		</div>';
				html += '	</div>';
				html += '</form>';


				$(self).append(html).css({"width":"780px","height":"64px"});
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '		<div class="uploadImg" style="width:90px">';
					html += '			<img id="uploadImage_'+file.index+'" class="upload_image" src="' + e.target.result + '" style="width:90px;" />';
					html += '		</div>';
					html += '	</a>';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onDelete: function(file, files) {
						...
						console.info("更新UI，当前待上传总数为："+ZYFILE.funReturnNeedFiles().length);
					},
			...
			this.addEvent = function(){
				...
				$(".upload_btn").bind("click", function(e){
					// 判断当前是否有文件需要上传
					if(ZYFILE.funReturnNeedFiles().length > 0&&ZYFILE.funReturnNeedFiles().length<=12){
						$(".fileSubmit").click()
					}
				});
