var ZYFILE = {
	...
	onFailure : function(file, responseInfo) { // 提供给外部获取单个文件上传失败，供外部实现失败效果
		alert("上传失败：file:" + file + ",info:" + responseInfo);
	},
	...
	funDeleteFile : function(delFileIndex, isCb) {
		console.info("删除的文件index："+delFileIndex);
		console.info("删除前，uploadFile："+this.uploadFile.length+",perUploadFile："+this.perUploadFile.length);
		...
		console.info("删除后，uploadFile："+this.uploadFile.length+",perUploadFile："+this.perUploadFile.length);
		return true;
	},
	// 上传多个文件
...
//		});
		for(var i=0;i<this.uploadFile.length;i++){
			self.funUploadFile(this.uploadFile[i]);
			console.info("上传文件："+this.uploadFile[i]+",time:"+new Date());
		}
	},
	// 上传单个个文件
	funUploadFile : function(file) {
		...
		xhr.addEventListener("error", function(e) {
			alert("图片上传失败！错误信息："+xhr.responseText);
			// 回调到外部
