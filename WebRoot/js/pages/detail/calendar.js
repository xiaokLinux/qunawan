/**
 * 文件注释：
 * 旅游产品详情中，日历控件的第三方框架，主要用于控制价格选择时，弹出的日历组件
 * 2015.12
 */

/**
 * 订单确认提交时调用
 * @param id 进行订单确认时对应产品的id 
 */
function takeOrders(id){
	// 从页面隐藏域获取旅游人数
	var num = $(".young").val();
	if(num==null || id==null || date==null){
		alert("请选择出行日期!");
		return;
	}
	$("#num").val(num);
...
var pickerEvent = {
    Init: function (elemid) {
        ...
        document.getElementById("calendar_choose").style.left = (getAbsoluteLeft(elemid)+3)+"px";
        document.getElementById("calendar_choose").style.top  = (getAbsoluteTop(elemid)-6) +"px";
        ...
        for (var i = 1; i <= 42; i++) {
            ...
            if ((i - 1) >= week && (i - c) <= days) {
				...
				if(obj.year==new Date().getFullYear()&&obj.month==new Date().getMonth()+1&&i-c==new Date().getDate()){
					html += '<td  ' + classStyle + ' date="' + obj.year + "-" + obj.month + "-" + (i - c) + '" price="' + price + '" style="background-color:#fae;box-shadow:inset 2px 2px 8px #c8a0cc"><a style="color:#fff"><span class="date basefix">今天</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';
				}
				else{
