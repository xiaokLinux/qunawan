	...
	<div class="content_all">
		...
		<div class="content_left ">
			<div class="content_left_first">
				<img src="user_img/1.jpg" class="img_head" />
				...
				<ul>
					...
					<li id="personal_detail" class="backgroundClass"><span
						...
						class="position_my_re_list"> <a
							href="personalinfo.jhtml"> 个人信息 </a>
					</span></li>
		...
		<div class="content_right_pa">
			...
			<div class="content_right" id="personal_detail_content">
				...
				<div class="per_detail_content">
					<form name="personForm" action="person!update" method="post" namespace="/">
						<ul>
							<li><span style="margin-left: 30px" class="redcolor"
								id="error_msg">${requestScope.err_msg}</span></li>

							<li><label> 昵称：</label> <input type="text"
								placeholder="请输入昵称" id="myname" name="user.name"
								value="${user.name }" class="input_style" /> <span
								id="myname_msg"></span></li>
							<li><label><span class="redcolor">*</span>性别：</label> 
								<c:choose>
							...
							</label> <input type="text" name="user.phone" placeholder="请输入手机号码"
							...
							</label> <input type="text" placeholder="请输入邮箱" id="email" name="user.email"
								value="${user.email }" class="input_style" onBlur="checkEmail()" />
								<span id="email_msg"></span></li>
							<li><label> 真实姓名： </label> <input type="text"
								placeholder="请输入真实姓名" id="realname" value="${user.realName }"
								name="user.realName" class="input_style" /> <span id="realname_msg"></span></li>
							<c:choose>
								<c:when test="${empty user.birthday.month}">
									<li><label> 生日：</label> <select class="sel_year" rel=""
										name="user.birthday.year"></select> <select class="sel_month" rel=""
										name="user.birthday.month"></select> <select class="sel_day" rel=""
										name="user.birthday.day"></select></li>
								</c:when>
								...
								<c:otherwise>
									<li><label> 生日：</label> <select class="sel_year"
										...
										name="user.birthday.year"></select> <select class="sel_month"
										rel="${user.birthday.month+1}" name="user.birthday.month"></select> <select
										class="sel_day" rel="${user.birthday.date}" name="user.birthday.day"></select></li>
								</c:otherwise>
							</c:choose>

							<li><label> 地区： </label> <select id="province" class="cityclass" onChange="checkCity()" onBlur="checkSelect()">
									<c:choose>
										<c:when test="${!empty user.city }">
											<option value='<s:property value="user.getCity().getId()" />' selected><s:property value="%{user.city.name}"  /></option>
										</c:when>
										<c:otherwise>
							...
							<select id="select_city" name="city" class="city cityclass" onBlur="checkSelectCity()">
									<option value='<s:property value="user.getCity().getId()" />' selected><s:property value="user.getCity().getName()" /></option>
...
>
						<div class="headMain">
							...
							<div class="source_pane">
								...
								<div>
									<a href="javascript:void(0);" class="file">上传照片 <input type="file"
									 name="image" id="imgOne" accept=".png,.jpg,.gif"
								>
									</a>
								</div>
							</div>
